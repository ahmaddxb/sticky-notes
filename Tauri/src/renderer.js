...
if (window.__TAURI__) {
    ...
    window.electronAPI = {
        logToServer: (msg) => invoke('log_server', { msg }),
        onLoadNote: (callback) => {
            const label = currentWindow.label;
            const currentNoteId = label.replace('note-', '');
            invoke('request_note_hydration', { id: currentNoteId }).then((note) => {
                callback(note);
            }).catch(err => {
                console.error('[Tauri Bridge] Load error:', err);
            });
            listen('load-note', (event) => {
                if (event.payload && event.payload.id === currentNoteId) {
                    callback(event.payload);
                }
            });
        },
        saveName: (id, name) => invoke('save_name', { id, name }),
        newNote: () => invoke('new_note'),
        closeNote: (id) => invoke('close_note', { id }),
        deleteNotePermanent: (id) => invoke('delete_note_permanent', { id }),
        saveContent: (id, content) => invoke('save_content', { id, content }),
        resizeWindow: (width, height) => {
            const label = currentWindow.label;
            const currentNoteId = label.replace('note-', '');
            invoke('resize_window', { id: currentNoteId, width, height });
        }
    };
}
